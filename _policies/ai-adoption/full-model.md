...
*Rationale: Two existing models demonstrate that evidence-based rulemaking is institutionally feasible. Utah's Artificial Intelligence Policy Act (2024) provides the most operationally specific AI sandbox to date — use-case waivers, upfront liability assignment, mandatory data collection, feedback into permanent rulemaking. Canada's Centre for Regulatory Innovation (CRI, est. 2018) provides a complementary meta-regulatory model: rather than running individual AI pilots, it builds regulators' institutional capacity to experiment across domains, with a Regulators' Experimentation Toolkit and cross-department collaboration mechanisms. Together they illustrate the full architecture: Utah shows what a specific AI sandbox looks like in practice; Canada shows what the supporting institutional infrastructure looks like. The CRI model is more scalable but less focused; the Utah model is more targeted but requires bespoke agreements for each deployment. A mature AI governance sandbox would benefit from both — dedicated AI scope (Utah) combined with shared capability infrastructure (CRI). Key design principles: liability assigned upfront; waivers use-case specific, not categorical; findings public; sandbox produces governance improvements, not permanent exemptions.*
